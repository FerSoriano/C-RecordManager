...
def create_node(id: int, name: str, author: str):
...
def search_book_by_id(root, id):
    book = {}
    def get_book_data(id, name, author):
        book["id"] = id
        book["name"] = name.decode('utf-8')
        book["author"] = author.decode('utf-8')
    c_lib.searchNodeById(root, id, CALLBACK(get_book_data))
    return book
...
def delete_book_by_id(root, id):
    return c_lib.deleteNode(root, id)